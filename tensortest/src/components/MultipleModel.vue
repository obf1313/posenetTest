...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getPointList: function (results,imageElement) {
                let pointList = [];
                for(let i=0; i<results.length; i++) {
                    for(let j=0; j<results[i].keypoints.length; j++) {
                        if(results[i].keypoints[j].score>0.5) {
                            pointList.push(results[i].keypoints[j].position);
                        }
                    }
                }
        ...
        mounted() {
            ...
            pose.then((result) => {
                console.log(result);
                this.getPointList(result,imageElement);
            });
        }
    }
</script>
